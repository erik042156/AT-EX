...
**작성 일자**: 2026-07-22
**최종 수정일**: 2026-07-22 (v1.3: 실제 화면 스크린샷 확인 결과로 (1) 비로그인 상태 `/logout` URL 직접 접근 시 노출되는 화면이 사용자 친화적 에러 안내 페이지가 아니라 Django 프레임워크의 디버그 예외 트레이스백 페이지(`KeyError at /logout`)임을 확정, (2) 다른 페이지에서의 "Logout" 링크 위치가 메인 페이지와 완전히 동일함(헤더가 전 페이지 공통 고정 영역)을 확정 반영)
**버전**: 1.3
**상태**: 초안 (로그아웃 성공 흐름 중심으로 범위 확정. 비로그인 상태에서 `/logout` URL 직접 접근 시 Django 디버그 트레이스백 페이지가 노출되는 흐름을 비정상 시나리오(ABN-002)로 자동화 대상에 포함. Locator는 Page Object 구현 착수 시 사용자가 실시간으로 공유할 예정)
...
**설명(URL 직접 접근 관점, 확인 완료 — v1.2 반영, v1.3 세부 확정)**: 다만 UI 진입점 여부와 별개로, Selenium `driver.get()` 등으로 로그아웃 URL(`https://automationexercise.com/logout`)에 **직접 접근**하는 것은 일반적인 자동화 테스트 범위에 해당합니다. 실제 화면 스크린샷 확인 결과, 비로그인 상태에서 이 URL에 직접 접근하면 사용자 친화적 에러 안내 페이지가 아니라 **Django 프레임워크의 디버그 예외 트레이스백 페이지**(헤딩 `KeyError at /logout`, 예외 값 `'user_id'`)가 그대로 노출됨이 확인되었습니다 (확인 완료, v1.3).

이에 따라 이 흐름은 예외 상황이 아니라고 단정하지 않고, 다음과 같이 **비정상 시나리오로 자동화 대상에 포함**합니다:

- **ABN-002**: 비로그인 상태에서 `/logout` URL 직접 접근 → Django 디버그 트레이스백 페이지(`KeyError at /logout`) 노출 확인 (섹션 11, 14, 15, 16 참고)
...
| **비로그인 상태에서 `/logout` URL 직접 접근** | ✅ 포함 (ABN-002, v1.2 도입, v1.3 세부 확정) | 로그인하지 않은 상태에서 로그아웃 URL(`https://automationexercise.com/logout`)에 직접 접근 시 Django 디버그 트레이스백 페이지(`KeyError at /logout`)가 노출되는지 확인 |
...
| **로그인하지 않은 상태에서 헤더 "Logout" 링크를 통한 로그아웃 시도** | 로그아웃은 로그인의 후행 기능으로, 로그인하지 않은 상태에서는 헤더에 "Logout" 링크 자체가 노출되지 않아 UI 진입점을 통한 시나리오는 성립하지 않음 (섹션 9 흐름 3 참고). 단, URL 직접 접근 시나리오는 ABN-002로 자동화 대상에 신규 포함됨 (v1.2, 섹션 11 참고) |
...
### ABN-002: 비로그인 상태에서 `/logout` URL 직접 접근 (v1.2 도입, v1.3 세부 확정, 자동화 대상 포함)

**시나리오명**: 비로그인(게스트) 상태에서 로그아웃 URL(`https://automationexercise.com/logout`)에 직접 접근

**전제조건**:
- 로그인되어 있지 않은 상태(게스트 상태)

**테스트 단계**:
1. 로그인하지 않은 상태에서 로그아웃 URL(`https://automationexercise.com/logout`)에 직접 접근 (예: Selenium `driver.get()`, 예시 수준이며 실제 코드 아님)
2. 페이지 로드 완료 대기

**기대 결과**:
- Django 디버그 트레이스백 페이지 노출 — 헤딩 `KeyError at /logout`, 예외 값 `'user_id'`가 화면에 표시됨 (확인 완료 — 실제 화면 스크린샷 확인 결과, v1.3). 이는 사용자 친화적으로 설계된 안내 페이지가 아니라, 서버가 세션에 없는 `user_id` 키를 삭제하려다 처리되지 않은 예외가 그대로 노출되는 **서버 측 미처리 예외(unhandled exception) 화면**입니다. 화면에는 Request Method(GET), Request URL, Django Version(3.2.8), Exception Type(KeyError) 및 `website/views.py, line 216, in logout` → `del request.session['user_id']` 코드가 포함된 traceback도 함께 노출됩니다.

**검증 포인트**:
- ✅ 페이지에 "KeyError at /logout" 텍스트가 포함되는가? (페이지 헤딩)
- ✅ 페이지에 예외 값 `'user_id'`가 포함되는가?
- ✅ (선택) 페이지에 Exception Type "KeyError" 텍스트가 포함되는가?

**리스크(확인 필요)**: 이 화면은 Django의 `DEBUG` 설정 값에 따라 노출 형태가 달라지는 프레임워크 차원의 디버그 화면입니다. 현재는 `DEBUG=True`로 추정되는 상태에서 위와 같은 트레이스백이 노출되는 것으로 확인되었으나, 실제 운영 환경에서 `DEBUG=False`로 전환되면 이 화면 대신 일반적인 500 에러 페이지가 노출될 수 있습니다. 이 경우 위 검증 포인트(트레이스백 텍스트 검증)는 더 이상 유효하지 않게 되므로, 운영 환경 설정 값 확인 및 그에 따른 검증 로직 재조정이 향후 필요합니다.

**비고**: 로그아웃 기능은 단일 링크 클릭으로 수행되는 액션이며 별도의 입력 필드가 없어, 로그인 기능과 같은 형태의 "잘못된 입력값" 기반 비정상 시나리오는 존재하지 않습니다. 이번 PRD의 비정상 시나리오는 ABN-001(자동화 범위 제외로 결정됨)과 ABN-002(자동화 대상 신규 포함)이며, 실제 자동화 테스트 케이스는 ABN-002에 대해서만 작성합니다.
...
### 비정상 케이스 기대 결과 (자동화 대상, v1.2 도입, v1.3 세부 확정)

| 시나리오 | 페이지 상태 | 메시지/표시 |
|---------|-----------|-----------|
| **ABN-002** | Django 디버그 트레이스백 페이지 | 비로그인 상태에서 `/logout` URL 직접 접근 시 헤딩 "KeyError at /logout", 예외 값 `'user_id'`가 표시되는 Django 디버그 예외 화면 노출 (확인 완료, v1.3 — DEBUG 설정에 따른 운영 환경 노출 형태 변화 가능성은 섹션 14 리스크 참고) |
...
### 비정상 시나리오(ABN-002) 검증 (v1.2 도입, v1.3 세부 확정)

```
테스트 흐름:
1. 로그인하지 않은 상태에서 `https://automationexercise.com/logout` URL 직접 접근 (driver.get() 등)
2. 페이지 로드 완료 대기
   → 검증: 페이지에 "KeyError at /logout" 텍스트가 포함되는가? (Django 디버그 트레이스백 페이지 헤딩)
   → 검증: 페이지에 예외 값 `'user_id'`가 포함되는가?

※ 리스크(확인 필요): 이 화면은 Django의 DEBUG 모드 설정 여부에 따라 노출 형태가 달라지는 프레임워크 차원의 디버그 화면입니다.
   실제 운영 환경에서 DEBUG=False로 전환되면 이 화면 대신 일반 500 에러 페이지가 노출될 수 있어, 위 검증 기준이
   달라질 수 있습니다 (섹션 14 ABN-002, 섹션 20 참고).
```
...
# ABN-002 예시 (v1.2 도입, v1.3 세부 확정, 실제 코드 아님)
# 사전 준비: 로그인하지 않은 상태(새 세션/드라이버)
driver.get("https://automationexercise.com/logout")

error_page = ErrorPage(driver)  # 클래스명은 예시이며 실제 구현 시 확정
# Django 디버그 트레이스백 페이지 노출 여부 확인
assert error_page.is_django_debug_traceback_displayed() == True  # "KeyError at /logout" 텍스트 포함 여부 등
# 참고(리스크): DEBUG=False 운영 환경에서는 이 화면 대신 일반 500 에러 페이지가 노출될 수 있어
# 위 메서드/검증 기준은 향후 운영 환경 확인 후 재조정이 필요할 수 있음 (섹션 14, 20 참고)
```
...
- ✅ **Logout 링크의 실제 요청 URL**: `https://automationexercise.com/logout` (실제 사이트 확인 완료). 로그인 상태에서만 정상 동작하며, 비로그인 상태에서 이 URL에 직접 접근하면 **Django 디버그 트레이스백 페이지가 노출됨**이 확인됨 (섹션 4, 9 흐름 3, 11, 14 ABN-002 반영, 세부 내용은 v1.3에서 확정)
...
- **Django DEBUG 설정에 따른 화면 노출 형태 변화 가능성**: 비로그인 상태 `/logout` URL 직접 접근 시 노출되는 Django 디버그 트레이스백 페이지(`KeyError at /logout`)는 `DEBUG=True` 설정 하에서 노출되는 것으로 추정됨. 실제 운영 환경에서 `DEBUG=False`로 전환되면 이 화면 대신 일반적인 500 에러 페이지가 노출될 수 있으며, 이 경우 섹션 14 ABN-002의 검증 기준(트레이스백 텍스트 검증)을 재조정해야 함 — 향후 운영 환경 설정 값 확인 필요
- HTTP 상태 코드(예: 500 등)의 정확한 값 자체는 화면 텍스트만으로는 확정하지 않았으므로, 필요 시 향후 확인 (다만 이번 자동화 대상 시나리오는 화면 텍스트 기반 검증을 기준으로 하므로 필수 확인 항목은 아님)
...
- ✅ ABN-002 (비로그인 상태에서 `/logout` URL 직접 접근 → 에러 페이지 노출, 신규 v1.2): 테스트 PASS
...
4. **테스트 코드 작성**: 로그아웃 기능 테스트 케이스 작성 (tests/test_logout.py) — 정상 시나리오(NOR-001, NOR-002) 및 비정상 시나리오(ABN-002: 비로그인 상태 `/logout` URL 직접 접근) 포함, 로그인 사전 준비용 공용 fixture 정의
5. **테스트 실행**: pytest를 사용하여 정상 시나리오(NOR-001, NOR-002) 및 비정상 시나리오(ABN-002) 테스트 실행
...
### 버전 관리
- **이 PRD 버전**: 1.3 (실제 화면 스크린샷 확인 결과로 섹션 20의 "확인 필요" 항목 2건 확정 반영)
- **작성일**: 2026-07-22
- **최종 검토일**: 2026-07-22
- **최종 승인일**: (예정)
