...
**작성 일자**: 2026-07-22
**최종 수정일**: 2026-08-03 (v1.5: 세션 타임아웃 시간을 Playwright MCP 실측으로 확정(14일/1,209,600초). 다만 사용자가 "값만 확정, 자동화는 영구 제외"를 명시적으로 선택함에 따라 "세션 만료 후 로그아웃 시도" 흐름(섹션 9 흐름 2)은 값 확정 이후에도 계속 자동화 대상에서 제외됨)
**버전**: 1.5
**상태**: 초안 (로그아웃 성공 흐름 중심으로 범위 확정. 비로그인 상태에서 `/logout` URL 직접 접근 시 Django 디버그 트레이스백 페이지가 노출되는 동작은 실제로 확인되었으나, Django `DEBUG` 설정에 의존하는 환경 종속적 시나리오라는 이유로 자동화 대상에서 제외하기로 결정함(구 ABN-002, 섹션 12 참고). 세션 타임아웃 시간(14일)은 실측으로 확정되었으나, 관련 자동화(세션 만료 후 로그아웃 시도)는 영구적 스코프 결정에 따라 계속 제외됨(섹션 9 흐름 2, 섹션 12 참고). Locator는 Page Object 구현 착수 시 사용자가 실시간으로 공유할 예정)
...
### 흐름 2: 세션 만료 후 로그아웃 시도 (자동화 범위에서 영구 제외됨, v1.5 스코프 결정)

**설명**: 세션이 이미 만료된 상태에서 "Logout" 링크를 클릭하는 흐름입니다. 세션 타임아웃 시간 자체는 v1.5에서 Playwright MCP 실측으로 **14일(1,209,600초)로 확정**되었습니다(project-prd.md 19.1 근거, `sessionid` 쿠키의 `expires` 값과 로그인 시각을 직접 비교하여 확인, Django 기본 `SESSION_COOKIE_AGE`와 소수점 단위까지 일치하여 Django 기본값으로 추정됨). 그러나 값이 확정된 이후에도 이 흐름의 자동화는 **여전히(그리고 이번에는 영구적으로) 자동화 범위에서 제외**하기로 결정하였습니다. 이는 "값을 몰라서" 보류하던 기존 상태와 달리, **값을 알고 있음에도 자동화하지 않기로 확정한 스코프 결정**입니다. 제외 사유: (1) 14일을 실제로 대기하는 E2E 테스트는 현재 CI 파이프라인(1회 실행이 수 분 내로 완료됨)에서 비현실적이며, (2) `sessionid` 쿠키를 직접 조작(삭제·변조)하여 만료를 시뮬레이션하는 방식도 LOGOUT-REQ-005("세션/쿠키 값 자체는 요구사항 대상 외 — 로그인/로그아웃 상태 판단은 오직 화면 텍스트/링크 기준")와 정합적으로 이번에는 채택하지 않기로 사용자가 명시적으로 결정하였습니다(섹션 12, 섹션 20 참고).
...
| **세션 타임아웃/만료 후 동작 검증** | ~~세션 타임아웃 시간 자체가 프로젝트 전체 차원에서 미확정. 확정 전까지 자동화 대상에서 제외~~ → **값 확정(v1.5) 이후에도 제외 유지**: 세션 타임아웃 시간은 14일(1,209,600초)로 실측 확정되었으나(project-prd.md 19.1 근거), (1) 14일 대기 E2E가 CI에서 비현실적이고 (2) 쿠키 조작 시뮬레이션도 LOGOUT-REQ-005 원칙과 배치되어 채택하지 않기로 함에 따라, "값 미확정이라 제외"에서 "값은 알지만 자동화가 비현실적이라 영구 제외"로 사유가 바뀐 채 계속 제외됨 (스코프 결정, v1.5. 섹션 9 흐름 2 참고) |
...
### 확인 완료 항목 (v1.5, Playwright MCP 실측으로 신규 확정)

- ✅ **세션 타임아웃 시간**: `/login` 접속 직후(로그인 전) 쿠키를 확인한 결과 `automationexercise.com` 도메인에는 `csrftoken`만 있고 `sessionid`는 없어(로그인 전에는 세션 쿠키가 발급되지 않음), 쿠키를 초기화한 뒤 사전 준비된 테스트 계정(`test_data/accounts.json`의 `shared_login_accounts.valid_account`)으로 실제 로그인을 수행하고, 로그인 클릭 직전 시각(Unix epoch, `beforeClick=1785748031`)과 로그인 직후 발급된 `sessionid` 쿠키의 `expires` 값(`1786957631.791659`)을 직접 비교함. 계산 결과 `expires - beforeClick = 1209600.79초`로, 정확히 **1,209,600초(14일)**임이 확인됨. 이는 Django 프레임워크의 기본 `SESSION_COOKIE_AGE` 설정값(2주)과 소수점 단위까지 정확히 일치하며, 이 사이트가 Django 3.2.8을 사용한다는 기존 확인 사실(섹션 4, `KeyError at /logout` 트레이스백 근거)과도 정합적이므로 Django 기본값으로 **추정**됨(서버 소스 코드 자체는 볼 수 없으므로 확정은 아님). 로그인 폼에는 "로그인 상태 유지(remember me)" 등 세션 기간에 영향을 줄 수 있는 별도 옵션이 없음을 스냅샷으로 확인하여, 모든 로그인이 동일하게 14일 고정 만료임을 확인함. `sessionid` 쿠키는 `httpOnly=true`, `secure=false`, `sameSite=Lax`(값 예시는 `9x6ohvbj8kbh7xr9z9gzeldbb2yt1aeu`)로 확인됨(확인 완료, v1.5, project-prd.md 19.1 근거). 이 실측 방법의 한계로, 서버가 별도의 idle timeout이나 세션 스토어 만료를 추가로 두었는지, 그리고 이 만료가 고정 만료(로그인 시점 + 14일)인지 매 요청마다 갱신되는 sliding expiration인지는 브라우저 쿠키 만료 시각 실측만으로는 완전히 배제하지 못함(Django 기본 설정은 `SESSION_SAVE_EVERY_REQUEST=False`이므로 고정 만료일 가능성이 높으나 확정하지 않음). **다만 값이 확정되었음에도 불구하고, 이 값을 이용한 자동화(섹션 9 흐름 2)는 사용자의 명시적 선택에 따라 영구적으로 자동화 범위에서 제외되었으므로 아래 "조사 대상에서 제외하기로 확정한 항목"에 함께 기록함.**
...
- **세션 만료 후 로그아웃 시도(섹션 9 흐름 2)**: 세션 타임아웃 값(14일/1,209,600초)은 v1.5에서 Playwright MCP 실측으로 확정되었으나(위 "확인 완료 항목 (v1.5)" 참고), 이 값을 근거로 한 실제 자동화는 사용자가 "값만 문서에 확정하고 자동화는 영구 제외"를 명시적으로 선택함에 따라 계속 제외됨 (스코프 결정, v1.5. 섹션 9 흐름 2, 섹션 12 참고). 제외 사유: (1) 14일을 실제로 대기하는 E2E 테스트는 CI 파이프라인에서 비현실적임(현재 CI 1회 실행은 수 분 내로 완료됨), (2) LOGOUT-REQ-005("세션/쿠키 값 자체는 요구사항 대상 외")와 정합적으로, `sessionid` 쿠키를 직접 조작(삭제·변조)해 만료를 시뮬레이션하는 방식도 이번에는 채택하지 않기로 결정함. 이는 "값이 미확정이라 보류"하던 이전 상태와 달리, 값을 알고 있는 상태에서 자동화하지 않기로 확정한 **영구적인 스코프 결정**입니다(섹션 9 흐름 1의 브라우저 뒤로가기 흐름 제외 결정과 동일한 성격).

### 여전히 확인이 필요한 항목

#### 프로젝트 전체 차원의 미확정 사항
- ~~세션 타임아웃 시간: 로그인 후 세션이 유지되는 시간이 얼마인지 프로젝트 전체 차원에서 아직 확정되지 않음~~ → **확인 완료(v1.5)**: Playwright MCP 실측으로 14일(1,209,600초)로 확정됨 (project-prd.md 19.1 근거, 위 "확인 완료 항목 (v1.5)" 참고). 값은 확정되었으나 세션 만료 관련 로그아웃 동작(섹션 9 흐름 2)의 자동화는 별도의 사유로 영구 제외되었음(위 "조사 대상에서 제외하기로 확정한 항목" 참고)
- 해당 없음 (그 외 프로젝트 전체 차원의 미확정 사항은 이 기능 관점에서 추가로 없음)
...
3. ~~세션 타임아웃 확정 후 확장 검토: 프로젝트 전체 차원에서 세션 타임아웃 시간이 확정되면, 세션 만료 후 로그아웃 동작(섹션 9 흐름 2) 시나리오를 추후 별도 범위로 추가할지 검토~~ → **확정 완료(v1.5)**: 세션 타임아웃 시간은 14일(1,209,600초)로 실측 확정되었으나, 사용자가 "값만 확정하고 자동화는 영구 제외"를 명시적으로 선택함에 따라 세션 만료 후 로그아웃 동작(섹션 9 흐름 2) 시나리오는 추가하지 않기로 최종 결정됨. 더 이상 "확정 대기 중"인 항목이 아니므로 이 다음 단계 항목은 종료됨
...
### 버전 관리
- **이 PRD 버전**: 1.5 (세션 타임아웃 시간을 실측 확정하되, 관련 자동화는 영구 제외로 스코프 확정함)
- **작성일**: 2026-07-22
- **최종 검토일**: 2026-08-03
- **최종 승인일**: (예정)

**변경 이력**:
- v1.5 (2026-08-03): `docs/ROADMAP.md` Phase 3 완료 조건에 마지막으로 남아있던 미해결 항목("세션 타임아웃 값 확정")을 사용자 승인 하에 Playwright MCP로 실측(CLAUDE.md §6 절차 준수)하여 확정함. 로그인 클릭 직전 시각(Unix epoch, `beforeClick=1785748031`)과 로그인 직후 발급된 `sessionid` 쿠키의 `expires`(`1786957631.791659`) 값을 직접 비교한 결과 **1,209,600초(14일)**로 확인되었으며, Django 기본 `SESSION_COOKIE_AGE`(2주)와 소수점 단위까지 일치해 Django 기본값으로 추정됨(서버 소스는 볼 수 없어 idle timeout 추가 여부·sliding expiration 여부까지는 배제하지 못하는 방법상 한계를 명시). 다만 이 값을 실제 자동화(섹션 9 흐름 2 "세션 만료 후 로그아웃 시도")에 활용할지 사용자에게 물은 결과, 사용자는 **"값만 문서에 확정하고 자동화는 영구 제외"**를 명시적으로 선택함(14일 대기 E2E가 CI에서 비현실적이라는 점, 쿠키 직접 조작을 통한 만료 시뮬레이션도 LOGOUT-REQ-005 원칙과 배치되어 채택하지 않기로 한 점이 사유). 이에 따라 (1) 섹션 4 헤더의 상태 요약, (2) 섹션 9 흐름 2의 설명을 "값 미확정이라 제외"에서 "값은 확정되었으나 영구 제외"로 갱신, (3) 섹션 12 자동화 제외 범위 표의 "세션 타임아웃/만료 후 동작 검증" 행을 취소선 처리 후 확정 상태로 갱신하되 제외 결정 자체는 유지, (4) 섹션 20에 "확인 완료 항목 (v1.5)"를 신설하여 실측 근거·한계를 기록하고, "조사 대상에서 제외하기로 확정한 항목"에 세션 만료 흐름을 추가하며, "여전히 확인이 필요한 항목"의 세션 타임아웃 관련 서술을 취소선 처리, (5) 섹션 22의 "세션 타임아웃 확정 후 확장 검토" 다음 단계 항목을 확정 완료로 종료함(섹션 4, 9, 12, 20, 22 반영). project-prd.md §19.1도 동일한 실측 결과와 스코프 결정으로 동기화되었으며(v1.4 → v1.5), 이번 개정은 실측 결과의 문서화 및 스코프 확정이며 Selenium/pytest 실제 동작 코드는 추가하지 않음.
