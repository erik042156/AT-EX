...
**작성 일자**: 2026-07-20  
**최종 수정일**: 2026-07-21 (테스트 계정 준비 방식을 사전 생성으로 확정)  
**버전**: 1.2  
...
### 진입 경로 (사용자 흐름)
1. **홈 페이지에서 진입**: 홈 페이지의 "Login" 네비게이션 링크 클릭
2. **로그아웃 후 진입**: 로그아웃 후 "Login" 링크 클릭
...
│ Home | Products | Login | Logout    │  (네비게이션)
├─────────────────────────────────────┤
│                                     │
...
**기대 결과**:
- 로그인 성공 시 메인 페이지로 이동하며, 헤더(네비게이션) 영역에 "Logout"이 노출됨
- 개인화된 사용자 영역 접근 가능
...
| **로그인 상태 표시** | ✅ 포함 | 로그인 후 메인 페이지 헤더에 "Logout" 노출 확인 |
...
- 메인 페이지 헤더(네비게이션) 영역에 "Logout"이 노출됨
...
**기대 결과**:
- 로그아웃 후 "Login" 링크 표시
- 재로그인 성공, 메인 페이지 헤더에 "Logout" 다시 노출

**검증 포인트**:
- ✅ 로그아웃 후 "Login" 링크가 표시되었는가?
...
| 시나리오 | 페이지 상태 | 메시지/표시 |
|---------|-----------|-----------|
| **NOR-001** | 메인 페이지(`/`) | 헤더(네비게이션) 영역에 "Logout" 노출 |
| **NOR-002** | 메인 페이지(`/`) | 헤더(네비게이션) 영역에 "Logout" 노출 |
...
| 용도 | 이메일 | 비밀번호 | 상태 | 비고 |
|------|--------|---------|------|------|
| **정상 로그인 테스트** | `test001@example.com` | `password123` | 활성 | 미리 생성된 계정 |
| **재로그인 테스트** | `test002@example.com` | `securepass456` | 활성 | 미리 생성된 계정 |
...
| 기능 | 의존 관계 | 설명 |
|------|---------|------|
| **회원가입 (Signup)** | 선행 필수 | 로그인 전에 계정이 먼저 생성되어야 함 |
| **홈 페이지 (Home)** | 진입 경로 | 홈 페이지의 "Login" 링크를 통해 로그인 페이지 진입 |
...
### 즉시 작업 (이 PRD 이후)
1. **테스트 계정 준비**: 사전 생성 방식으로 테스트용 이메일과 비밀번호 생성/확인 (테스트 실행 중 동적 생성 방식은 사용하지 않음)
2. **Locator 공유 대기**: Page Object 구현 착수 시 사용자로부터 각 UI 요소("Email Address", "Password" 입력란, "Login" 버튼, 오류 메시지 요소)의 id, name, CSS 클래스, data-* 속성을 실시간으로 공유받기
...
### 버전 관리
- **이 PRD 버전**: 1.2 (테스트 계정 준비 방식을 사전 생성으로 확정)
- **작성일**: 2026-07-20
- **최종 검토일**: 2026-07-21
- **최종 승인일**: (예정)
